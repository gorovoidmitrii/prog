...
int apple = 0;
int n = 0;
int v1 = 20; // скорость для поворота
int	v2 = 50; // скорость для езды прямо
void dooro ()
{
...
task distance() //расстояние до препятствия
{
	while (true)
	{
		if(SensorValue[sDis]<30)
		{
			...
			{
				n = 1; //флаг остановки = стоять
				v1 = 0; //обнуление скорости
				v2 = 0;
			}
			else
			{
...
task main()
{
	...
	while(true)
	{
		if ( apple == 0)
		{
			...
			{// слишком маленькое расстояние, надо отьехать
				motor[mB]=v2;
				motor[mA]=v2;
				sleep(1);
				nMotorEncoder[mB]=0;
