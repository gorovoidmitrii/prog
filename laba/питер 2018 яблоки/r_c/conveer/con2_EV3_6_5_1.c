...
float const MAX_DISTANCE_2_BIG_APPLE = 18.5;//19.5;//21.5
...
int getColour()
{
	...
	*/
	...
	sort();
	int col = GREEN;
	if (redValue > greenValue) //(redValue >= MIN_RED_VALUE_FOR_RED_COLOUR)&&(greenValue <= MAX_GREEN_VALUE_FOR_RED_COLOUR))
	{
		if((redValue >= MIN_RED_VALUE_FOR_ORANGE_COLOUR)||(greenValue >= MIN_GREEN_VALUE_FOR_ORANGE_COLOUR))
		{
			col = ORANGE;
		}
		else
