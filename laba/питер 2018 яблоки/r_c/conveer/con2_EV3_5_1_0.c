...
int const DISTANCE_WITHOUT_APPLE = 28;//distance without apple
int const GREEN = 0;
int const RED = 1;
int const SMALL = 0;
int const BIG = 1;
...
bool isCourierHere = 0;//0- 1 or 2 courier go out, 1- both couriers stands
int isAppleLoaderLoading = 0;//vaisAppleLoaderLoadingiable for check is body open or no
bool isTransporterHere = 0;
bool determineByColor = 0;//dis=0,col=1
float const MAX_DISTANCE_2_BIG_APPLE = 23.5;//min dis to big apple
		...
		if (determineByColor == 0)
		{
			if (isAppleLoaderLoading == 1)
			{
				if(isAppleReady4Conveyer==1)
				{
					...
					if (appleSizeNow == BIG) //if apple is big
					{
						moveAppleToCourier();
					}
					else
