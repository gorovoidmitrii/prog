...
task main()
{
	...
	sleep(5000);*/
	 STOL = MSDISTreadDist(sLeft);
	sleep(100);
	GoodDist= MSDISTreadDist(sLeft);
	displayTextLine(2,"%d", GoodDist);
	playSound(soundBeepBeep)
	nMotorEncoder[mA] = 0;
	while (nMotorEncoder[mA]<500)//241//=275//=235
	{
		motor[mA]=20;
		motor[mB]=20;
	}
  ...
  errDist = abs(sort()-GoodDist);//abs(sort()-GoodDist);
	displayTextLine(3,"err = %d", errDist);
	int testLenght = 241;
	float howManyDegWeNeed2Turn = radiansToDegrees( atan2(errDist, testLenght));
	displayTextLine(1,"deg = %d",howManyDegWeNeed2Turn);
	turnHelp(howManyDegWeNeed2Turn);
	displayTextLine(4,"GoodDist = %d",GoodDist);
	displayTextLine(5," sort = %d",sort());
	playSound(soundBeepBeep);
	sleep(15000);
}
void turnHelp(float degrees)
{
	...
	if (GoodDist-dv>=0)
	{
		while (nMotorEncoder[mA] < encoders*-1)//241//=275//=235
		{
			motor[mA]=20;
			motor[mB]=-20;
		}
	...
	{
		while (nMotorEncoder[mA]>encoders)//241//=275//=235
		{
			motor[mA]=-20;
			motor[mB]=20;
		}
